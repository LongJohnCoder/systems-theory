...
static int module_open(struct inode *inode, struct file *file)
{
	 ...
	 * to the process. 
	 * wait_event_interruptible_locked checks the condition while holding the lock.
	 * If the condition is true, it returns. If not, it releases the lock and goes to sleep. 
	 * When woken up, it acquires the lock and checks the condition again.
	 * return code: -ERESTARTSYS if it was interrupted by a signal 
	 * and 0 if !AlreadyOpen evaluated to true. 
	 */

	res = wait_event_interruptible_locked(WaitQ, !Already_Open);
	/* 
