...
Revision History:
    ...
    Alex Ionescu - Started Implementation - 16-Feb-06
    Alex Ionescu - Completed Implementation - 19-Feb-06

--*/
#include "precomp.h"

/*++
 * @name VgaInterpretCmdStream
 *
 * The VgaInterpretCmdStream routine reads a specially formed "Command Stream"
 * (see bootdata.c) and inteprets the commands contained within.
 *
 * @param CmdStream
 *        Pointer to the Command Stream to interpret.
 *
 * @return TRUE if the Command Stream was interpreted successfully, FALSE if
 *         invalid commands were found.
 *
 * @remarks This routine is used to initialize the VGA registers for various
 *          modes.
 *
 *--*/
BOOLEAN
 ...
 * @name VgaIsPresent
 *
 * The VgaIsPresent routine verifies whether a VGA Adapter is present and
 * responding normally to read/write attemps to the VGA Registers.
 *
 * @param None.
 *
 * @return TRUE if detection was successful, FALSE otherwise.
 *
 * @remarks Because some VGA Cards do not properly respect the standard,
 *          every write is paranoidly checked with a read and compared.
 *
 *--*/
BOOLEAN
NTAPI
 ...
 * @name VidInitialize
 *
 * The VidInitialize routine initializes the boot video driver, and optionally
 * sets the video mode if requested.
 *
 * @param SetMode
 *        Specifies whether the video mode should be set, or only if the driver
 *        should be initialized.
 *
 * @return TRUE if the driver successfully initialized, FALSE otherwise.
 *
 * @remarks None.
 *
 *--*/
...
/*++
 * @name VidResetDisplay
 *
 * The VidResetDisplay routine resets the display
 *
 * @param HalReset
 *        Specifies if this reset should be a total VGA BIOS reset as well.
 *
 * @return None.
 *
 * @remarks Resetting the VGA will restore the mode and fill the screen black.
 *
 *--*/
VOID
NTAPI
